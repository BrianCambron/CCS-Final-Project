...
def send_sms(phone_number):
    ...
    for User in Profile.objects.all():
        for x in Receipt.objects.all():
            sum = Receipt.objects.all().aggregate(Sum('total_amount'))
            total = round(sum.get('total_amount__sum'), 2)
            break
        username = User.user
        phone_number = User.phone_number
        Usermessage = User.message

        good_message_1 = "Becoming rich is hard. Staying broke is hard. Choose your hard. \n – Eric Worre \n Hey, %s. You spent %s this week. You save amount of money week. \n" % (username, total)
        ...
        if User.phone_number:
            client.messages.create(
             body=good_message_3 + Usermessage,
             from_='+19382220702',
             to=phone_number
             )
